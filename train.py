...
def make_side_by_side(sketch, fake, photo, normalize=False):
    """
    Create a clean 3-channel comparison image. Works with normalized tensors in [-1, 1].
    """
...
def train(opt):
    # Add prefix for logging jobs – use experiment name
    ...
    def log_images_to_wandb(visuals, step, label=""):
        """
        Logs key visuals + comparison + grouped grid to WandB
        """
        images = []
    ...
    print(f"{log_prefix} The number of training images = {dataset_size}")
    ...
    for epoch in range(opt.epoch_count, max_epochs):
        ...
        for i, data in enumerate(dataset):
            ...
            # Log images + losses every N steps
            if total_iters % opt.print_freq == 0:
        ...
        print(
            f"{log_prefix} End of epoch {epoch} / {opt.niter + opt.niter_decay} Time Taken: {int(time.time() - epoch_start_time)} sec"
        )

        # Save the latest model checkpoint
        model.save_networks("latest")

        # Log visuals once per N epochs
