...
def get_norm_layer(norm_type="instance"):
    ...
    if norm_type == "batch":
        ...
        return lambda module: SpectralNorm(module)
    elif norm_type == "none":
...
def define_G(
    ...
    if netG == "resnet_9blocks":
        net = ResnetGenerator(
            ...
            n_blocks=9,
            self_attn_layer_indices=self_attn_layer_indices,
        )
...
class SelfAttnGenerator(nn.Module):
    ...
    def __init__(self, image_size=256, z_dim=100, conv_dim=256):
        ...
        self.attn1 = SelfAttn(
            int(curr_dim / 2), nn.ReLU(), forward_outputs_attention=True
        )
    ...
    def forward(self, z):
        ...
        )  # no idea what @heykeetae was going for here - is one of the 1's supposed to be a -1?
        out = self.l1(z)
        out = self.l2(out)
        out = self.l3(out)
    ...
    def __init__(
        ...
        for i in range(n_blocks):  # add ResNet blocks
            ...
            if i in self_attn_layer_indices:
                model += [
                    SelfAttn(ngf * mult, nn.ReLU(True), forward_outputs_attention=False)
                ]
...
class UnetGenerator(nn.Module):
    ...
    def __init__(
        ...
        if 0 in self_attn_layer_indices:
            unet_block = nn.Sequential(
                unet_block,
                SelfAttn(ngf * 8, nn.ReLU(True), forward_outputs_attention=False),
            )
        ...
        if num_downs - 3 in self_attn_layer_indices:
            unet_block = nn.Sequential(
                unet_block,
                SelfAttn(ngf * 4, nn.ReLU(True), forward_outputs_attention=False),
            )
        ...
        if num_downs - 2 in self_attn_layer_indices:
            unet_block = nn.Sequential(
                unet_block,
                SelfAttn(ngf * 2, nn.ReLU(True), forward_outputs_attention=False),
            )
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if (
                type(norm_layer) != functools.partial
                and norm_layer.__name__ == "SpectralNorm"
            ):
                sequence += [norm_layer(conv2d), leakyReLU]
            else:
                sequence += [conv2d, norm_layer(ndf * nf_mult), nn.LeakyReLU(0.2, True)]
            if n in self_attn_layer_indices:
                sequence.append(
                    SelfAttn(
                        ndf * nf_mult,
                        nn.ReLU(True),
                        forward_outputs_attention=False,
        ...
        if (
            ...
            sequence += [norm_layer(conv2d), leakyReLU, norm_layer(final_conv2d)]
        else:
            sequence += [conv2d, norm_layer(ndf * nf_mult), leakyReLU, final_conv2d]
